...
class VAE(torch.nn.Module):
    def __init__(self, state_dim, action_dim, vae_features, vae_layers, max_action=1.0):
        ...
        self.encoder = MLP(self.state_dim + self.action_dim, 2 * self.latent_dim, vae_features, vae_layers,
                           hidden_activation='relu')
        self.decoder = MLP(self.state_dim + self.latent_dim, self.action_dim, vae_features, vae_layers,
                           hidden_activation='relu')
...
class Jitter(torch.nn.Module):
    def __init__(self, state_dim, action_dim, jitter_features, jitter_layers, max_action=1.0, phi=0.05):
        ...
        self.jitter_net = MLP(self.state_dim + self.action_dim, self.action_dim, jitter_features, jitter_layers,
                              hidden_activation='relu')
...
def algo_init(args):
    ...
    jitter = Jitter(obs_shape, action_shape, args['jitter_features'], args['jitter_layers'], max_action,
                    args['phi']).to(args['device'])
    jitter_optim = torch.optim.Adam(jitter.parameters(), lr=args['jitter_lr'])

    q1 = MLP(obs_shape + action_shape, 1, args['value_features'], args['value_layers'], hidden_activation='relu').to(
        args['device'])
    q2 = MLP(obs_shape + action_shape, 1, args['value_features'], args['value_layers'], hidden_activation='relu').to(
        args['device'])
        ...
        for epoch in range(self.args['max_epoch']):
            for i in range(self.args['steps_per_epoch']):
                ...
                with torch.no_grad():
                    ...
                    target_q = self.lam * torch.min(target_q1, target_q2) + (1 - self.lam) * torch.max(target_q1,
                                                                                                       target_q2)
